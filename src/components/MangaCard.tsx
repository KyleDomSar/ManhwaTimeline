...
export function MangaCard({manga,onPress}:Props){
 return <Pressable onPress={onPress} style={({pressed})=>[styles.card,pressed&&styles.pressed]}>
  {manga.coverUrl?<Image source={{uri:manga.coverUrl}} style={styles.cover}/>:<View style={[styles.cover,styles.placeholder]}><Text style={styles.placeholderText}>No Cover</Text></View>}
...
const styles=StyleSheet.create({card:{flexDirection:"row",backgroundColor:colors.surface,borderRadius:radius.md,overflow:"hidden",borderWidth:1,borderColor:colors.border,marginBottom:spacing.md},pressed:{opacity:.75},cover:{width:92,height:128},placeholder:{alignItems:"center",justifyContent:"center",backgroundColor:colors.surfaceElevated},placeholderText:{color:colors.textMuted,fontSize:11},info:{flex:1,padding:spacing.md,gap:spacing.sm},title:{color:colors.text,fontSize:17,fontWeight:"800"},badges:{flexDirection:"row",alignItems:"center",gap:spacing.sm},status:{color:colors.primary,fontSize:12,textTransform:"capitalize",fontWeight:"700"},meta:{color:colors.textSecondary,fontSize:12},genres:{color:colors.textSecondary,fontSize:12},followers:{color:colors.textMuted,fontSize:11},chapter:{color:colors.textMuted,fontSize:11}});
